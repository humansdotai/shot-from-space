...
export interface ImageryRates {
  ...
  /** $/km² for the cheapest priced high-res archive scene over the target. */
...
export function imageryUsd(tier: PricingTier, rates: ImageryRates, areaKm: number) {
  ...
  if (tier === 'ARCHIVE') {
    const billed = Math.max(footprint, rates.archiveMinKm2);
    return {
      usd: rates.archivePerKm2 * billed,
      billedKm2: billed,
      note: `${rates.archiveProvider} archive scene · $${rates.archivePerKm2}/km² × ${trim(billed)} km²`,
    };
  }
